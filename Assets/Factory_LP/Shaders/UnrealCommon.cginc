...
struct FStrataTree
{
	...
	FStrataData SubstrateWeight(FStrataData A, float Weight, int OperatorIndex, uint MaxDistanceFromLeaves)
	{
		return A;
	}
	FStrataData SubstrateVerticalLayeringParameterBlending(FStrataData Top, FStrataData Base, uint NewNormalIndex, float NoV)
	{
		//...
		return Top;
	}
	FStrataData SubstrateVerticalLayering(FStrataData Top, FStrataData Base, int OperatorIndex, uint MaxDistanceFromLeaves)
	{
		//...
		return Top;
	}
...
//5.4
#define FSubstrateData FStrataData
#define FSubstrateTree FStrataTree
#define SubstrateTree StrataTree
#define SubstratePixelFootprint StrataPixelFootprint

#define SHAREDLOCALBASIS_INDEX_0 0
#define SHAREDLOCALBASIS_INDEX_0_0 0
#define SHAREDLOCALBASIS_INDEX_0_1 0
#define SHAREDLOCALBASIS_INDEX_1_0 0
...
MaterialFloat3 ReflectionAboutCustomWorldNormal( FMaterialPixelParameters Parameters, MaterialFloat3 WorldNormal, bool bNormalizeInputNormal )
{
	...
	return reflect( Parameters.CameraVector, WorldNormal );//9.10.2024 - Works well with the CameraVector calculation from ShaderGraph
...
{
	//Need to return Shared because otherwise it goes over the 16 sampler limit when sampling over 16 textures
...
MaterialFloat4 Texture2DSample(Texture2D Tex, SamplerState Sampler, float2 UV)
{
	...
	#if HDRP || URP
		#if RAYTRACING
			return SAMPLE_TEXTURE2D_LOD( Tex, Sampler, UV, 0 );
		#else
...
//Fixed Texture2DSample coming from CustomExpressions compiled in VertexShaders
MaterialFloat4 Texture2DSampleForVS( Texture2D Tex, SamplerState Sampler, float2 UV )
{
#if HDRP || URP
	return SAMPLE_TEXTURE2D_LOD( Tex, Sampler, UV, 0 );
#else
...
MaterialFloat4 TextureCubeSample( TextureCube Tex, SamplerState Sampler, float3 UV)
{
	...
	#if HDRP || URP
		#if RAYTRACING
			return SAMPLE_TEXTURECUBE_LOD( Tex, Sampler, UV, 0 );
		#else
...
MaterialFloat4 TextureCubeSampleGrad(TextureCube Tex, SamplerState Sampler, float3 UV, MaterialFloat3 DDX, MaterialFloat3 DDY)
{
			...
			return SAMPLE_TEXTURECUBE_LOD( Tex, Sampler, UV, 0 );
		#else
			return SAMPLE_TEXTURECUBE(Tex, Sampler, UV);//, DDX, DDY);
		#endif
	#else
		return texCUBE(Tex, UV);
	#endif
...
MaterialFloat4 Texture2DSampleBias( Texture2D Tex, SamplerState Sampler, float2 UV, float MipBias )
{
	...
	MaterialFloat4  Texture3DSample( Texture3D Tex, SamplerState Sampler, float3 UV )
	{
		UV.y = 1.0 - UV.y;
		#if RAYTRACING
			return SAMPLE_TEXTURE3D_LOD( Tex, Sampler, UV, 0 );
		#else
			return SAMPLE_TEXTURE3D( Tex, Sampler, UV );
		#endif
	}
	MaterialFloat4  Texture3DSampleLevel( Texture3D Tex, SamplerState Sampler, float3 UV, float Mip )
	{
		UV.y = 1.0 - UV.y;
		return SAMPLE_TEXTURE3D_LOD( Tex, Sampler, UV, Mip );
	}
	...
	{
		#if RAYTRACING
			return SAMPLE_TEXTURECUBE_ARRAY_LOD( Tex, Sampler, UV.xyz, UV.w, 0 );
		#else
	...
	MaterialFloat4  Texture2DArraySampleLevel( Texture2DArray Tex, SamplerState Sampler, float3 UV, float Mip )
	{
		UV.y = 1.0 - UV.y;
		return SAMPLE_TEXTURE2D_ARRAY_LOD( Tex, Sampler, UV.xy, UV.z, Mip );
	}	
#endif
...
{
	float2 PixelPos = BufferUV.xy * View_BufferSizeAndInvSize.xy - View.ViewRectMin.xy;
...
FPrimitiveSceneData GetPrimitiveData( FMaterialVertexParameters Parameters )
{
	...
	FLWCVector3 GetWorldCameraOrigin( FMaterialVertexParameters Parameters )
	{
		...
		return ResolvedView.WorldCameraOrigin;
	}
	...
	#define GetPrevWorldCameraOrigin GetWorldCameraOrigin
	float3 GetDistanceFieldGradientGlobal( FLWCVector3 WorldPosition )
	{
		float3 Gradient = float3( 0, 0, 0.001f );
		return Gradient;
	}
	MaterialFloat MaterialExpressionNoise( FLWCVector3 Position, float Scale, int Quality, int Function, bool bTurbulence, uint Levels, float OutputMin, float OutputMax, float LevelScale, float FilterWidth, bool bTiling, float RepeatSize )
	{
		return MaterialExpressionNoise( LWCToFloat(Position), Scale, Quality, Function, bTurbulence, Levels, OutputMin, OutputMax, LevelScale, FilterWidth, bTiling, RepeatSize );
	}
	FLWCInverseMatrix GetWorldToLocal( FMaterialVertexParameters Parameters )
	{
		return GetPrimitiveData( Parameters ).WorldToLocal;//Parameters.LWCData.WorldToLocal;
	}
...
{
#if HDRP || URP
	return float4( SHADERGRAPH_SAMPLE_SCENE_COLOR( ScreenUV.xy ), 0.0f );
#else
...
{
	return Parameters.PostProcessUV;
}
#if URP
	TEXTURE2D(_BlitTexture);//Needs to be without _X so builds for OculusQuest don't error in SceneTextureLookup
#elif HDRP
...
float3 MaterialExpressionSkyAtmosphereViewLuminance( FMaterialPixelParameters Parameters, float3 Vec3 = float3(0,0,0))
{
#if MATERIAL_SKY_ATMOSPHERE && PROJECT_SUPPORT_SKY_ATMOSPHERE
	//...
#else
...
{
	//was inverted Y but this flips UVs in PostProcessShaders
	return float2( ViewportUV.x, ViewportUV.y );
}
...
//5.5
float VTPageTableUniform_Unpack( float x )
{
    return 0;
}
...
FStrataData GetStrataSlabBSDF(
	...
	float ThicknessCm,
	bool bIsThinSurface,
	uint SharedLocalBasisIndex, uint SharedLocalBasisTypes )
{
	FStrataData Out;
	Out.EmissiveColor = Emissive;
	Out.Opacity = 1;
...
{
	FStrataData Out;
	Out.EmissiveColor =  float3(0,0,0);
	Out.Opacity = 1;
	Out.BaseColor = float3(1,1,1);
	Out.Metallic = 0;
	Out.Roughness = 1;
...
#define GetInitialisedSubstrateData GetInitialisedStrataData
...
//5.4
//For convenience assuming WSVECTOR_IS_TILEOFFSET
...
#define GetSubstrateSingleLayerWaterBSDF GetStrataSingleLayerWaterBSDF
#define FSubstratePixelFootprint FStrataPixelFootprint
...
FSubstrateData GetSubstrateSlabBSDF(
	...
	float3 DiffuseAlbedo, float3 F0, float3 F90,
	float Roughness, float Anisotropy,
	float SSSProfileID, float3 SSSMFP, float SSSMFPScale, float SSSPhaseAnisotropy, uint SSSType,
	float3 Emissive,
	float SecondRoughness, float SecondRoughnessWeight, float SecondRoughnessAsSimpleClearCoat,
	float ClearCoatUseSecondNormal, float3 ClearCoatBottomNormal,
	float FuzzAmount, float3 FuzzColor, float FuzzRoughness,
	float GlintValue, float2 GlintUV,
	float SpecularProfileId,
	float ThicknessCm,
...
{
	float SafeWeight = saturate(Weight);
...
#define GetParticleWorldPosition GetWorldPosition
#define GetPrevParticleWorldPosition GetPrevWorldPosition
...
#define SubstrateConvertLegacyMaterialStatic StrataConvertLegacyMaterialStatic
...
FStrataData StrataConvertLegacyMaterialStatic(
	...
	float3 Emissive,
	float Opacity,
	float3 ThinTranslucentTransmittanceColor, float ThinTranslucentSurfaceCoverage,
	float3 WaterScatteringCoefficients, float3 WaterAbsorptionCoefficients, float WaterPhaseG, float3 ColorScaleBehindWater,
	uint ShadingModel,
	float3 RawNormal,
	float3 RawTangent,
...
//From here on there's only new code found in higher versions of unreal anyway
#ifdef UE5
FWSVector3 GetPreViewTranslation(FMaterialVertexParameters Parameters)
{
    return ResolvedView.PreViewTranslation;
}
...
float4x4 DFFastToTranslatedWorld(FLWCInverseMatrix WorldToLocal, FLWCVector3 PreViewTranslation)
{
	//Demote
    return LWCToFloat( WorldToLocal ); //DFFastMultiplyTranslationDemote(DFNegate(PreViewTranslation), WorldToLocal);
}
float4x4 DFFastToTranslatedWorld(FLWCMatrix WorldToLocal, FLWCVector3 PreViewTranslation)
{
	//Demote
    return LWCToFloat(WorldToLocal); //DFFastMultiplyTranslationDemote(DFNegate(PreViewTranslation), WorldToLocal);
}
...
FWSVector2 DFToWS(FDFVector2 In)
{
    return LWCPromote( In.High );
}
FWSVector3 DFToWS(FDFVector3 In)
{
    return LWCPromote( In.High );
}
FWSVector4 DFToWS(FDFVector4 In)
{
    return LWCPromote( In.High );
}
FDFVector2 DFConstructor(float2 High, float2 Low)
{
	FDFVector2 Result;
	Result.High = High;
	Result.Low = Low;
...
FDFVector2 DFPromote(float2 Value) { return DFConstructor(Value, (float2)0); }
...
{
    FWaterBodyData Data;
    Data.WaterZoneIndex = 0;
    Data.FixedVelocity = float3(0, 0, 0);
    Data.FixedZHeight = 0.0;
    Data.FixedWaterDepth = 0.0;
    return Data;
}
...
{
    FWaterZoneData Data;
    Data.Location = float2( 0, 0 );
    Data.Extent = float2( 1, 1 );
    Data.HeightExtent = float2( 1, 1 );
    Data.GroundZMin = 0;
    return Data;
}
...
{
	#ifdef UE5
		return LWCToFloat( GetActorWorldPosition(Parameters) );
	#else
		return GetActorWorldPosition(Parameters);
	#endif
}
...
float3x3 DFToFloat3x3(FLWCInverseMatrix Mat)
{
    return LWCToFloat( Mat );
}
...
{
	#ifdef UE5
		return mul(Parameters.AbsoluteWorldPosition, LWCToFloat( GetWorldToInstance(Parameters)) );
	#else
		return mul(Parameters.AbsoluteWorldPosition, GetWorldToInstance(Parameters));
	#endif
}

float3 GetPositionInstanceSpace(FMaterialVertexParameters Parameters)
{
    return Parameters.WorldPosition; //PositionInstanceSpace;
}
...
float2 Append( float x, float y )
{
	return float2( x, y );
}
float3 Append( float A, float2 B )
{
	return float3( A, B.xy );
}
float3 Append( float2 A, float B )
{
	return float3( A.xy, B );
}
float4 Append( float2 A, float2 B )
{
	return float4( A.xy, B.xy );
}
float4 Append( float3 A, float B )
{
	return float4( A.xyz, B );
}
float4 Append( float A, float3 B )
{
	return float4( A, B.xyz );
}

#define GetPrevPreViewTranslation GetPreViewTranslation
#define GetInstanceToWorldDF GetInstanceToWorld
#define GetWorldToLocalDF GetWorldToLocal
#define GetPrevWorldToLocal GetWorldToLocal
#define GetWorldToInstanceDF GetWorldToInstance
#define GetPositionPrimitiveSpace_NoMaterialOffsets GetPositionPrimitiveSpace
...
float4 GetSceneTextureViewSize(uint SceneTextureId)
{
    return View.ViewSizeAndInvSize;
}
...
{
	float3 Resolution = Uniforms.VolumePageResolution * float(SPARSE_VOLUME_TILE_RES);
	float3 TexCoordDDX = UVWDDX * Resolution;
	float3 TexCoordDDY = UVWDDY * Resolution;
	float DDXLengthSquared = dot(TexCoordDDX, TexCoordDDX);
	float DDYLengthSquared = dot(TexCoordDDY, TexCoordDDY);
	float MaxLengthSquared = max(DDXLengthSquared, DDYLengthSquared);
	float ComputedMipLevel = 0.5f * log2(max(MaxLengthSquared, 1e-8f));
...
//#endif
	...
	float FinalMipLevelClamped = max(0.0f, ComputedMipLevel + MipBias + Noise);
	return FinalMipLevelClamped;
}
float3 SparseVolumeTextureSamplePageTable(/*Texture3D<uint>*/ float PageTable, FSparseVolumeTextureUniforms Uniforms, float3 UVW, uint AddressU, uint AddressV, uint AddressW, int MipLevel = 0)
{
    return float3(0, 0, 0);
}
float4 SparseVolumeTextureSamplePhysicalTileData(float PhysicalTileDataA, float PhysicalTileDataB, float TileDataSampler, float3 VoxelUVW, int PhysicalTileDataIndex)
{
    return float4(0, 0, 0, 0);
}
float4 SparseVolumeTextureSamplePhysicalTileDataSecondMipWrapper(float PhysicalTileDataA, float PhysicalTileDataB, float TileDataSampler, float3 VoxelUVW, int PhysicalTileDataIndex)
{
    return float4(0, 0, 0, 0);
}
float3 SparseVolumeTextureSamplePageTableSecondMipWrapper(float PageTable, FSparseVolumeTextureUniforms Uniforms, float3 UVW, uint AddressU, uint AddressV, uint AddressW, int MipLevel = 0)
{
    return float3(0, 0, 0);
}
